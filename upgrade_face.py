# Upgrades the faces.super_face metarig to use the new skin face system.
#
# Preserves the deform bone layout (i.e. skinning is still valid), but
# control names and behavior aren't 100% compatible.
...
def list_face_bones(obj):
    ...
    def rec(bone):
        print('process(%r)' % (bone.name))
        for child in bone.children:
            rec(child)

    rec(obj.data.bones[face_bone])
